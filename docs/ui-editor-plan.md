...
| 缺陷 | 症状 | 修法 |
|---|---|---|
| `delete-state` 走 `window.confirm` | 对话框弹在**浏览器窗口**里（用户可能根本没看那边），而模态框会**阻塞渲染进程**——面板的指令与快照轮询一起停摆，直到有人找到那个框 | 确认搬到点击发生的那一侧：面板的「删除冻结页」改成**两步按钮**（首击变 saffron + 「再点一次删除」，5s 自动解除），指令带 `confirmed: true`；页内按钮保留自己的对话框 |
| `panelHost.close()` 无界等待 | `server.close()` 会等在途连接，卡住的指令能把 `shutdown()`、进而整个会话退出挂住 | 等待加 500ms 上限 + `closeAllConnections()` |
| `/events` 握手处 `JSON.parse` 未加保护 | 该处是未 await 的 async 回调，抛错变成未处理拒绝 → **进程退出**而不是流断开 | 包进 try/catch，解析失败就置空等下一轮 |
| `editor-field` 可写入下拉框没有的取值 | `select.value` 静默变 `""`，空 scope 被写进 intent | 对 SELECT 校验取值必须在选项内 |
| `swiftc` 构建无超时 | 编译器挂住会拖住 `await finished`，产物已写好但浏览器与中继不退出 | 120s 上限，超时 kill 并按构建失败降级 |
| 面板「建立分组」与文本指令并发发送 | 名字还没送达服务器，分组已被判"请先填写分组名称" | 指令等前面的文本请求返回后再发 |

**审查中刻意不改的一点**：`__SYMBUI_PANEL_COMMAND__` 是页面全局，页面脚本可以调 `{command:"finish"}` 结束会话。这**不是新增能力**——shadow root 是 open 的，同一段脚本今天就能点页内面板的「完成并生成」；加 nonce 也只是抬高门槛（页面可以包裹全局读到 nonce）。宿主页面是用户自己的本地应用，因此保留该入口，并把这条边界写进文档而不是留给意外。
